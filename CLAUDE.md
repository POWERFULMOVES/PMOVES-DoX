...
### Geometric Intelligence ("Mathematical UI")
...
**CHIT Protocol** (Cymatic Holographic Information Theory):
...
- API: `GET /cipher/geometry/demo-packet` - Demo CGP structure

**Curvature Detection**:
- Analyzes embedding distributions to determine manifold shape:
  - **Hyperbolic** (negative curvature): Tree-like hierarchies, high variance
  - **Spherical** (positive curvature): Compact clusters, low variance
  - **Euclidean** (flat): Intermediate distributions
- Generates `chit_manifold.json` configs for visualization

**Frontend Visualizers** (`frontend/components/geometry/`):
- `HyperbolicNavigator.tsx`: Poincaré disk visualization for hierarchical knowledge
  - Uses D3.js for 2D hyperbolic projections
  - Displays "super nodes" with nested constellations
  - Connected to NATS geometry bus for real-time updates
- `Manifold3D.tsx`: Three.js 3D surface rendering
  - Auto-rotating camera with OrbitControls
  - Dynamic geometry based on manifold parameters
- `ZetaVisualizer.tsx`: Riemann Zeta spectral analysis
  - Animates concentric "ripples" based on zeta zero frequencies
...
- Mounted: `http://localhost:8000/hyperdimensions` (static files)
- Provides additional geometric visualization tools
...
- `GeometryEngine` (`app/services/geometry_engine.py`): Curvature analysis for manifold detection
- `ChitService` (`app/services/chit_service.py`): NATS-based geometry packet handling
...
- `cipher.py`: Byterover Cipher memory/skill integration + geometry endpoints
...
- `geometry/`: Geometric Intelligence components
  - `HyperbolicNavigator.tsx`: Poincaré disk hierarchical visualization
  - `Manifold3D.tsx`: Three.js 3D manifold renderer
  - `ZetaVisualizer.tsx`: Riemann Zeta spectral visualization
...
- `NATS_URL`: NATS server URL for geometry bus (default: `nats://nats:4222`)
...
**NATS Geometry Bus**:
- Real-time geometry packet streaming between backend and frontend
- Used for live updates to HyperbolicNavigator and other geometric visualizations
- Stream: `GEOMETRY` with subjects `tokenism.cgp.>` and `geometry.>`
